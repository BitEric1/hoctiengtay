...
function HeroHeader() {
    return (
        <div className="relative overflow-hidden rounded-3xl ring-1 ring-white/15 shadow-[0_20px_60px_rgba(2,6,23,0.35)]">
...
function BlueSection({ id, icon, title, subtitle, children }) {
    return (
        <section
            id={id}
            className="relative overflow-hidden rounded-3xl ring-1 ring-white/20 shadow-[0_20px_60px_rgba(2,6,23,0.25)] text-white"
        >
            <div className="bg-[linear-gradient(180deg,#2563eb_0%,#60a5fa_100%)]">
                <div className="flex items-center gap-3 border-b border-white/10 px-6 py-4">
                    <div className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white/20 ring-1 ring-white/30">
                        {icon}
                    </div>
                    <div>
...
function FeatureCard({ title, desc, icon, pills, className }) {
    const cls = [
        "group relative overflow-hidden rounded-2xl p-5 text-white backdrop-blur-md",
        "bg-white/10 ring-1 ring-white/30 shadow-[0_14px_44px_rgba(2,6,23,0.35)] transition-[transform,box-shadow] duration-200 hover:-translate-y-0.5 hover:shadow-[0_18px_54px_rgba(2,6,23,0.45)]",
        className,
    ]
        .filter(Boolean)
        .join(" ");
    return (
        <div className={cls}>
            ...
            <p className="mt-1 text-sm text-white/95">{desc}</p>
            {pills && (
...
function StepCard({ title, desc, step }) {
    const cls = [
        "relative overflow-hidden rounded-2xl p-5 text-white backdrop-blur-md ring-1 ring-white/30 shadow-[0_12px_40px_rgba(2,6,23,0.3)]",
        "bg-white/10",
    ].join(" ");
    return (
        <div className={cls}>
...
function MemberCard({ m, onOpen, className }) {
    const cls = [
        "group relative w-full overflow-hidden rounded-2xl p-5 text-left text-white transition-transform duration-200",
        "bg-white/10 ring-1 ring-white/30 shadow-[0_12px_40px_rgba(2,6,23,0.28)] hover:-translate-y-0.5 hover:shadow-[0_18px_54px_rgba(2,6,23,0.4)]",
        className,
    ]
        .filter(Boolean)
        .join(" ");
        ...
        <button onClick={onOpen} className={cls}>
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div className="text-xs text-white/95">{m.role}</div>
                </div>
            </div>
            <p className="mt-3 line-clamp-3 text-sm text-white/95">
                “{m.quote}”
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
                {m.skills.map((s) => (
...
function MemberModal({ m, onClose, onPrev, onNext, index, total }) {
    return (
        <div className="fixed inset-0 z-50">
            ...
            <div className="absolute inset-x-0 bottom-0 mx-auto w-full max-w-5xl overflow-hidden rounded-t-3xl ring-1 ring-white/20 backdrop-blur-md sm:inset-0 sm:my-auto sm:rounded-3xl">
                <div className="grid grid-cols-1 md:grid-cols-2">
                    ...
                    {/* Info nửa phải */}
                    <div className="relative bg-[linear-gradient(180deg,#2563eb_0%,#60a5fa_100%)] p-8 text-white">
                        <button
                            onClick={onClose}
                        ...
                        <p className="text-sm text-white/95">{m.role}</p>

                        <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                            <div>
                                <dt className="text-white/80">Tuổi</dt>
                                <dd className="font-semibold">{m.age}</dd>
                            </div>
                            <div>
                                <dt className="text-white/80">Trường/Lớp</dt>
                                <dd className="font-semibold">{m.school}</dd>
                            </div>
                            <div>
                                <dt className="text-white/80">Thứ tự</dt>
                        ...
                        <blockquote className="mt-4 rounded-2xl bg-white/25 p-4 text-white ring-1 ring-white/40">
                            “{m.quote}”
                        </blockquote>
                        <p className="mt-4 text-white/95">{m.bio}</p>
...
if (typeof window !== "undefined") {
    ...
    console.assert(
        Array.isArray(reasons) && reasons.length >= 3,
        "reasons >= 3"
    );
    console.assert(
